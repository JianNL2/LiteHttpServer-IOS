...
public:
    HttpRequestParser()
        : state(RequestMethodStart), contentSize(0),
          chunkSize(0), chunked(false),request()
    ...
    ParseResult parse(const char *begin, const char *end)
    {
        return consume(request, begin, end);
    }
    ...
    ParseResult consume(HttpRequest &req, const char *begin, const char *end)
    {
        while( begin != end )
        {
            ...
            case PostBoundaryLine3:
                contentSize --;
                if (input == '\n')
                    state = PostBoundaryLine3_2;
                break;
            case PostBoundaryLine3_2:
                contentSize --;
                if (input == '\n')
                    state = Post;
                break;
